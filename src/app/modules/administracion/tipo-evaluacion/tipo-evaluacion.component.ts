...
import {ModalTipoEvacuacionComponent} from './modal-tipo-evacuacion/modal-tipo-evacuacion.component'
...
export class TipoEvaluacionComponent implements OnInit {
  ...
  constructor(private http:HttpClient,private toastr: ToastrService,
             ...
             private SidebarComponent:SidebarComponent) {
              ...
              this.SidebarComponent.menuspermiso(12).subscribe(resp=>{
  ...
  editar(item: any) {
    if(this.permisoactualizar){
      ...
      this._dialog.open(ModalTipoEvacuacionComponent, dialogConfig);
  ...
  impo() {
   ...
   <h5>REPORTE</h5>
   <h5>Listado  de Tipos Evaluación</h5>
   </div>
   </div><br>`;
    printJS({
      printable: 'reporte',
