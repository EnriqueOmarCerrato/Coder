...
import {ModalTipoInclusionComponent} from './modal-tipo-inclusion/modal-tipo-inclusion.component'
import * as printJS from 'print-js';
import { NavService } from 'src/app/services/nav.service';
import {SidebarComponent} from 'src/app/shared/components/sidebar/sidebar.component'

@Component({
  selector: 'app-tipo-inclusion',
  templateUrl: './tipo-inclusion.component.html',
  styleUrls: ['./tipo-inclusion.component.scss']
})
export class TipoInclusionComponent implements OnInit {
  loading=true;
  loadingConfig=true;
  mostrarModal=false;
  data:any;
  ...
  constructor(private http:HttpClient,private toastr: ToastrService,
             ...
             private SidebarComponent:SidebarComponent) {
              ...
              this.SidebarComponent.menuspermiso(13).subscribe(resp=>{
  ...
  editar(item: any) {
    if(this.permisoactualizar){
      ...
      this._dialog.open(ModalTipoInclusionComponent, dialogConfig);
  ...
  impo() {
   ...
   <h5>REPORTE</h5>
   <h5>Listado  de Tipos Inclusion</h5>
   </div>
   </div><br>`;
    printJS({
      printable: 'reporte',
